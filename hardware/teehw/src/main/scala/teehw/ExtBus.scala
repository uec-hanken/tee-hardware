...
{
  def instantiate(context: HasTileLinkLocations, loc: Location[TLBusWrapper])(implicit p: Parameters): ExtBus = {
    val crbus = LazyModule(new ExtBus(this, loc.name))
...
class ExtBus(params: ExtBusParams, name: String)(implicit p: Parameters)
